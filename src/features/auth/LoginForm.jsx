    ...
    return (
        <div className="flex flex-col items-center gap-5">
            ...
            <form className="flex flex-col gap-2" onSubmit={handleSubmit(handleSubmitForm)}>
                {loginRows.map(row => <InputRow className={authInputStyle} register={register} key={row.name} name={row.name} title={row.title} type={row.type} validateCondition={row.validateCondition} error={errors[row.name]?.message} info={row.info} />)}
